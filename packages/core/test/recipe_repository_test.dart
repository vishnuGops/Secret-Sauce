...
({RecordingHttpClient http, SupabaseClient client, SupabaseRecipeRepository repo})
...
void main() {
  group('getById', () {
    test('is ONE request carrying the embed and all four ascending orders',
        () async {
      final (:http, :client, :repo) = _repo([(200, jsonEncode(_recipeRow()))]);
      ...
      expect(recipe.ingredientGroups.single.ingredients.single.name, 'yoghurt');
  ...
  group('save', () {
    test('update is one save_recipe call with the content in list order',
        () async {
      final (:http, :client, :repo) = _repo([
        (200, jsonEncode('r1')), // the RPC returns the id
        (200, jsonEncode(_recipeRow(title: 'Renamed'))), // the getById after it
      ]);
      await signInAs(client, _uid);
    ...
    test('a 42501 from the RPC becomes WriteDeniedException', () async {
      ...
      await expectLater(
        repo.update(const Recipe(id: 'r1', ownerId: 'someone-else', title: 'x')),
